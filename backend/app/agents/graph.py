    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": _planner_payload(state)},
            ],
        )
        ...
        errors.append(f"planner_fallback: {exc}")
        trace = _append_trace(
            state,
            agent="planner",
            status="fallback",
...
def _next_agent(state: AgentGraphState) -> RouteName:
    ...
    for step in sorted(state.get("plan", []), key=lambda item: int(item.get("step", 0))):
...
def build_agent_workflow() -> Any:
    ...
    graph.add_node("reconciliation", make_placeholder_node("reconciliation"))
    graph.add_node("tax_compliance", make_placeholder_node("tax_compliance"))
    graph.add_node("merchant_ops", make_placeholder_node("merchant_ops"))
