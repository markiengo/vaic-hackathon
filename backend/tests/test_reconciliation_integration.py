...
from tests.p1_db_fixtures import disable_shared_seed_for_p1_tests as _p1_no_seed  # noqa: F401
from tests.p1_db_fixtures import p1_db_session_fixture as _p1_session  # noqa: F401
from tests.p1_db_fixtures import p1_truth_set_fixture as _p1_truth  # noqa: F401

_P1_FIXTURES = (_p1_no_seed, _p1_session, _p1_truth)
...
async def test_transaction_capacity_includes_legacy_null_sale_allocation(
    p1_db_session: AsyncSession,
    p1_truth_set: P1TruthSet,
):
    from app.models.transaction import BankTransaction

    sale_time = datetime(2026, 7, 15, 9, 30, tzinfo=timezone.utc)
    target_sale = Sale(
        id="P1-CAPACITY-TARGET",
        merchant_id=p1_truth_set.merchant_id,
        store_id=p1_truth_set.store_id,
    ...
    p1_db_session.add_all([target_sale, existing_sale, transaction])
    await p1_db_session.flush()
    ...
    plan = allocate_payment(
        ...
        (AllocationLeg(sale_id=target_sale.id, amount=Decimal("400000")),),
        (
            DomainSale(
                id=target_sale.id,
                merchant_id=p1_truth_set.merchant_id,
    ...
    assert error.value.code == "TRANSACTION_OVER_ALLOCATION"
    target_allocation = await p1_db_session.scalar(
        select(PaymentAllocation).where(
            PaymentAllocation.bank_transaction_id == transaction.id,
            PaymentAllocation.sale_id == target_sale.id,
        )
    )
    assert target_allocation is None
