    ...
    <div className="space-y-8">
      ...
      <div className="rounded-xl border bg-accent/30 p-5 sm:p-6">
        <p className="text-sm leading-6 text-text-secondary">
          <strong className="text-ink">Vai trò của SHB:</strong> Giám sát danh mục merchant, can thiệp khi agent cần con người quyết định, và kiểm toán toàn bộ thao tác ghi dữ liệu.
        </p>
      </div>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        <Card className="p-5">
          <p className="text-[13px] font-medium text-text-tertiary">Danh mục</p>
          <p className="mt-3 font-display text-3xl text-ink">{summary.total}</p>
        ...
        <Card className="p-5">
          <p className="text-[13px] font-medium text-text-tertiary">Cần can thiệp</p>
          <p className="mt-3 font-display text-3xl text-ink">{summary.open_cases + waiting.length}</p>
        ...
        <Card className="p-5">
          <p className="text-[13px] font-medium text-text-tertiary">Tuân thủ</p>
          <p className="mt-3 font-display text-3xl text-ink">{averageReadiness === "—" ? averageReadiness : `${averageReadiness}%`}</p>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.3fr_.7fr]">
        <div>
          <div className="mb-4 flex items-end justify-between"><div><p className="text-sm font-semibold text-secondary">Ưu tiên danh mục</p><h2 className="mt-1 font-display text-3xl text-ink">Merchant cần chú ý</h2></div><Link href="/ops/merchants" className="text-sm font-semibold text-secondary">Mở master-detail</Link></div>
        ...
        <Card>
          ...
          <h2 className="mt-2 font-display text-3xl text-ink">{waiting.length} run chờ quyết định</h2>
...
function CaseWorkspace({ caseId, compact = false }: { caseId: string; compact?: boolean }) {
  ...
  return (
    <div className={cn("space-y-6", !compact && "space-y-8")}>
      ...
      {compact && <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-xs text-text-secondary">{item.id}</p><h2 className="mt-1 font-display text-3xl text-ink">Case detail</h2></div><Link href={`/ops/cases/${item.id}`} className="inline-flex items-center gap-2 text-sm font-semibold text-secondary">Mở toàn trang<ArrowRight size={15} /></Link></div>}
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
        ...
        <Card className="h-fit">
          ...
          <h3 className="mt-4 font-display text-2xl text-ink">Decision controls</h3>
...
export function ComplianceView() {
  ...
  return <div className="space-y-8"><PageHeader eyebrow="Deterministic governance" title="Tuân thủ" description="Rule versions, effective periods, legal sources và approval ownership từ kho quy tắc backend." />{rules.isLoading && <LoadingState />}{rules.isError && <SectionError title="Không tải được tax rules" error={rules.error} retry={() => rules.refetch()} />}{rules.data && <><Card variant="information" className="flex items-start gap-4"><ShieldCheck aria-hidden className="mt-1 shrink-0 text-secondary" size={22} /><div><h2 className="font-display text-2xl text-ink">Rule engine là nguồn quyết định</h2><p className="mt-2 text-sm leading-6 text-text-secondary">Agent chỉ truy xuất và giải thích rule đã duyệt. Công thức, version và nguồn pháp lý không do mô hình tự tạo.</p><p className="mt-3 text-xs font-semibold text-secondary">{rules.data.rules.length} rule versions · {rules.data.rules.filter((rule) => rule.approval_status === "APPROVED").length} đã duyệt</p></div></Card><DataTable caption="Phiên bản tax rule" columns={ruleColumns} rows={rules.data.rules} getRowKey={(row) => row.version} empty={<EmptyState title="Chưa có rule version" description="Cần seed rule đã duyệt trước khi đánh giá readiness." />} /></>}</div>;
}
