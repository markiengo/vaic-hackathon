...
function ExceptionsWorkspace() {
  ...
  return (
    <div className="space-y-7 animate-[route-in_240ms_ease-out]">
      ...
      {/* Progress indicator */}
      <div className="flex items-center justify-between rounded-xl border bg-surface px-5 py-4"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-text-secondary">Tiến độ hàng chờ</p><p className="mt-1 text-sm"><strong>{queue.data.length}</strong> mục còn lại</p></div><div className="h-2 w-32 overflow-hidden rounded-full bg-border"><div className="h-full bg-secondary" style={{ width: `${Math.max(10, 100 - queue.data.length * 16)}%` }} /></div></div>
      <section className="grid gap-5 xl:grid-cols-[minmax(16rem,0.58fr)_minmax(32rem,1.42fr)]">
        ...
        <Card variant="workspace" className="p-0">
          <div className="border-b p-5 sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-text-secondary">Ngoại lệ #{selected.id}</p><h2 className="font-display mt-2 text-3xl">{selected.sender_name ?? "Người gửi chưa rõ"}</h2><p className="font-mono mt-2 text-2xl">{formatMoney(selected.amount)}</p></div><StatusPill status="Cần xác nhận" /></div></div>
          <div className="space-y-7 p-5 sm:p-7">
            <div className="rounded-xl border bg-background p-5"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-text-secondary">Nội dung ngân hàng</p><blockquote className="font-display mt-3 text-2xl">“{selected.raw_note ?? "Không có nội dung"}”</blockquote></div>
            ...
            <fieldset><legend className="font-display text-2xl">Khoản tiền này là gì?</legend><p className="mt-2 text-sm text-text-secondary">Chọn một phân loại. Quyết định chưa được lưu cho đến khi bạn bấm xác nhận.</p><div className="mt-4 grid gap-3 sm:grid-cols-2">{choices.map(([value, label, detail]) => { const isSuggested = selected.ai_suggestion?.suggested_type === value; return <button key={value} type="button" onClick={() => setClassification(value)} aria-pressed={classification === value} className={cn("relative min-h-24 rounded-xl border p-4 text-left hover:border-secondary", classification === value && "border-secondary bg-accent", isSuggested && !classification && "border-secondary/40 bg-accent/30")}><strong className="block text-sm">{label}</strong><span className="mt-1 block text-xs leading-5 text-text-secondary">{detail}</span>{isSuggested && <span className="absolute right-3 top-3 inline-flex items-center gap-1 rounded-full bg-secondary/10 px-2 py-0.5 text-[10px] font-semibold text-secondary"><Sparkles aria-hidden size={10} />Đề xuất</span>}{classification === value && <Check aria-hidden className="absolute right-3 top-3 text-secondary" size={18} />}</button>; })}</div></fieldset>
            <div className="flex flex-wrap justify-end gap-3 border-t pt-5"><Button variant="ghost" onClick={() => setClassification(undefined)} disabled={!classification}>Để sau</Button><Link href="/assistant" className="inline-flex min-h-11 items-center justify-center rounded-lg border px-4 text-sm font-semibold text-text-secondary hover:bg-background">Nhờ SHB hỗ trợ</Link><Button onClick={() => void confirm()} disabled={!classification || resolution.isPending}>{resolution.isPending ? "Đang lưu..." : "Xác nhận lựa chọn"}</Button></div>
          </div>
        </Card>
      </section>
    </div>
