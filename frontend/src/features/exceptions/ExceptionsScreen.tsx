      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(16rem,0.58fr)_minmax(32rem,1.42fr)]">
        ...
        <Card variant="workspace" className="p-0">
          ...
          <div className="space-y-7 p-5 sm:p-7">
            ...
            <fieldset><legend className="font-display text-2xl">Khoản tiền này là gì?</legend><p className="mt-2 text-sm text-text-secondary">Chọn một phân loại. Quyết định chưa được lưu cho đến khi bạn bấm xác nhận.</p><div className="mt-4 grid gap-3 sm:grid-cols-2">{choices.map(([value, label, detail]) => <button key={value} type="button" onClick={() => setClassification(value)} aria-pressed={classification === value} className={cn("relative min-h-24 rounded-xl border p-4 text-left hover:border-secondary", classification === value && "border-secondary bg-accent")}><strong className="block text-sm">{label}</strong><span className="mt-1 block text-xs leading-5 text-text-secondary">{detail}</span>{classification === value && <Check aria-hidden className="absolute right-3 top-3 text-secondary" size={18} />}</button>)}</div></fieldset>
