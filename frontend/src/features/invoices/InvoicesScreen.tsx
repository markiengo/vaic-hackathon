...
function InvoicesWorkspace() {
  ...
  return (
    <div className="space-y-7 animate-[route-in_240ms_ease-out]">
      ...
      <div className="flex items-center gap-4 rounded-xl border bg-surface px-5 py-4">
        <div className="flex-1">
          <div className="flex items-center justify-between text-sm">
            <span className="font-semibold text-ink">{Math.max(0, (query.data.total ?? allRecords.length) - query.data.missing_count)}/{query.data.total ?? allRecords.length} đơn đã có hóa đơn</span>
      ...
      {query.data.missing_count > 0 ? <Card className="grid gap-4 border-danger/25 sm:grid-cols-[auto_1fr_auto] sm:items-center"><span className="grid size-11 place-items-center rounded-full bg-danger/10 text-danger"><ReceiptText aria-hidden size={20} /></span><div><p className="font-display text-2xl text-ink">Còn {query.data.missing_count} đơn thiếu hóa đơn</p><p className="mt-1 text-sm text-text-secondary">Mỗi khoảng trống là một blocker trực tiếp của bộ xuất thuế. {Math.max(0, (query.data.total ?? allRecords.length) - query.data.missing_count)}/{query.data.total ?? allRecords.length} đơn đã có hóa đơn hợp lệ.</p></div><Link href={`/invoices?period=${period}&status=missing`} className="inline-flex min-h-10 items-center justify-center rounded-xl bg-primary px-4 text-sm font-semibold text-white hover:bg-primary-hover">Xử lý {query.data.missing_count} đơn</Link></Card> : <Card className="flex items-center gap-4 border-success/25"><span className="grid size-11 place-items-center rounded-full bg-success/10 text-success"><FileCheck2 aria-hidden size={20} /></span><div><p className="font-display text-2xl text-ink">Độ phủ hóa đơn đã hoàn tất</p><p className="mt-1 text-sm text-text-secondary">Mọi đơn đã thanh toán đều có bằng chứng chứng từ.</p></div></Card>}
      <div role="tablist" aria-label="Lọc hóa đơn" className="flex flex-wrap gap-2">{tabs.map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={status === value} onClick={() => { setStatus(value); setSelectedId(undefined); }} className={cn("min-h-10 rounded-xl border bg-surface px-4 text-sm font-semibold text-text-secondary hover:border-primary", status === value && "border-primary bg-primary text-white")}>{label}{value === "missing" && query.data.missing_count > 0 ? ` (${query.data.missing_count})` : ""}</button>)}</div>
