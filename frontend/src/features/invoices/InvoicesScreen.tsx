  ...
  return (
    <div className="space-y-7 animate-[route-in_240ms_ease-out]">
      ...
      <div className="flex items-center gap-4 rounded-xl border bg-surface px-5 py-4">
        <div className="flex-1">
          <div className="flex items-center justify-between text-sm">
            <span className="font-semibold text-ink">{Math.max(0, (query.data?.total ?? allRecords.length) - (query.data?.missing_count ?? 0))}/{query.data?.total ?? allRecords.length} đơn đã có hóa đơn</span>
            <span className="text-text-secondary">{query.data?.total ? Math.round(((query.data.total - (query.data.missing_count ?? 0)) / query.data.total) * 100) : 0}%</span>
          </div>
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-border">
            <div className="h-full bg-secondary transition-all" style={{ width: `${query.data?.total ? Math.round(((query.data.total - (query.data.missing_count ?? 0)) / query.data.total) * 100) : 0}%` }} />
          </div>
        </div>
      </div>

      {(query.data?.missing_count ?? 0) > 0 ? <Card className="grid gap-4 border-danger/25 sm:grid-cols-[auto_1fr_auto] sm:items-center"><span className="grid size-11 place-items-center rounded-full bg-danger/10 text-danger"><ReceiptText aria-hidden size={20} /></span><div><p className="font-display text-2xl text-ink">Còn {query.data?.missing_count ?? 0} đơn thiếu hóa đơn</p><p className="mt-1 text-sm text-text-secondary">Mỗi khoảng trống là một blocker trực tiếp của bộ xuất thuế. {Math.max(0, (query.data?.total ?? allRecords.length) - (query.data?.missing_count ?? 0))}/{query.data?.total ?? allRecords.length} đơn đã có hóa đơn hợp lệ.</p></div><Link href={`/invoices?period=${period}&status=missing`} className="inline-flex min-h-10 items-center justify-center rounded-xl bg-primary px-4 text-sm font-semibold text-white hover:bg-primary-hover">Xử lý {query.data?.missing_count ?? 0} đơn</Link></Card> : <Card className="flex items-center gap-4 border-success/25"><span className="grid size-11 place-items-center rounded-full bg-success/10 text-success"><FileCheck2 aria-hidden size={20} /></span><div><p className="font-display text-2xl text-ink">Độ phủ hóa đơn đã hoàn tất</p><p className="mt-1 text-sm text-text-secondary">Mọi đơn đã thanh toán đều có bằng chứng chứng từ.</p></div></Card>}
      <div role="tablist" aria-label="Lọc hóa đơn" className="flex flex-wrap gap-2">{tabs.map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={status === value} onClick={() => { setStatus(value); setSelectedId(undefined); }} className={cn("min-h-10 rounded-xl border bg-surface px-4 text-sm font-semibold text-text-secondary hover:border-primary", status === value && "border-primary bg-primary text-white")}>{label}{value === "missing" && (query.data?.missing_count ?? 0) > 0 ? ` (${query.data?.missing_count})` : ""}</button>)}</div>
      {records.length === 0 ? <EmptyState title="Không có hồ sơ trong bộ lọc này" description="Đổi bộ lọc để xem các đơn hàng và hóa đơn liên quan." /> : <section className="grid gap-5 xl:grid-cols-[minmax(18rem,0.72fr)_minmax(30rem,1.28fr)]">
        ...
        {selected && <Card variant="workspace" className="p-0"><div className="border-b p-5 sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[13px] font-medium text-text-tertiary">Quan hệ đơn hàng · thanh toán · hóa đơn</p><h2 className="font-mono mt-3 text-2xl">{selected.sale_id}</h2><p className="font-display mt-2 text-3xl text-ink">{formatMoney(selected.amount)}</p></div><StatusPill status={selected.readiness_blocker ? "Thiếu hóa đơn" : "Đạt"} /></div></div><div className="space-y-6 p-5 sm:p-7">
