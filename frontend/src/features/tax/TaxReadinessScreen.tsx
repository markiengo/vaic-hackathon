...
function TaxReadinessWorkspace() {
  ...
  return (
    <div className="space-y-7 animate-[route-in_240ms_ease-out]">
      ...
      <section><div className="mb-4"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-text-secondary">Bộ dữ liệu nháp</p><h2 className="font-display mt-2 text-3xl">Xuất để kiểm tra</h2></div><Card className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center"><div><h3 className="text-sm font-semibold">Không phải hành động nộp thuế</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-text-secondary">Các tệp chứa giao dịch, phân bổ, phân loại, hóa đơn và phiên bản quy tắc đã dùng. Tệp MISA chỉ được tải xuống, không tự động gửi sang MISA.</p></div><div className="grid gap-2 sm:grid-cols-3">{([ ["json", "JSON"], ["csv", "CSV"], ["misa_csv", "MISA CSV"] ] as Array<[TaxExportFormat, string]>).map(([format, label]) => <Button key={format} variant={format === "misa_csv" ? "primary" : "outline"} disabled={!report.export_allowed || Boolean(exporting)} onClick={() => void exportData(format)}>{report.export_allowed ? <Download aria-hidden size={16} /> : <LockKeyhole aria-hidden size={16} />}{exporting === format ? "Đang tạo..." : label}</Button>)}</div></Card></section>
    </div>
  );
}
