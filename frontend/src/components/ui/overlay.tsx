...
function NativeDialog({ open, onOpenChange, title, description, children, footer, sheet = false }: OverlayProps & { sheet?: boolean }) {
  ...
  return (
    <dialog
      ...
      className={cn(
        "surface-shadow-lg m-auto max-h-[calc(100dvh-2rem)] w-[min(36rem,calc(100%-2rem))] rounded-2xl border bg-surface p-0 text-ink backdrop:bg-brand-navy/55 open:animate-[route-in_180ms_ease-out]",
