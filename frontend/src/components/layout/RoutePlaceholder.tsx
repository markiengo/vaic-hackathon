  ...
  return (
    <section className="animate-[route-in_180ms_ease-out]">
      <div className="flex flex-col gap-5 border-b pb-7 sm:flex-row sm:items-end sm:justify-between">
        ...
        <button type="button" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-primary px-5 text-sm text-white transition-[background-color,transform] duration-150 ease-out hover:-translate-y-0.5 hover:bg-primary-hover active:scale-[0.97]">
      ...
      <div className="mt-7 grid gap-5 lg:grid-cols-[1.55fr_0.85fr]">
        <article className="surface-shadow min-h-[430px] rounded-xl border bg-surface p-5 sm:p-7">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.14em] text-text-tertiary">Không gian làm việc</p>
              <h2 className="mt-2 text-xl font-normal text-text">Dữ liệu tháng 07/2026</h2>
            </div>
            <span className="rounded-full border bg-accent px-3 py-1.5 text-xs text-text">Đang đồng bộ</span>
          </div>
          <div className="mt-8 grid gap-3">
            {["Nguồn dữ liệu đã kết nối", "Bằng chứng đã được chuẩn hóa", "Hành động cần người dùng xác nhận"].map((label, index) => (
              <div key={label} className="flex min-h-16 items-center gap-4 rounded-lg border bg-background px-4">
        ...
        <aside className="rounded-xl border bg-surface p-5 sm:p-7">
          <p className="text-xs uppercase tracking-[0.14em] text-text-tertiary">Trạng thái sản phẩm</p>
          <p className="font-display mt-5 text-6xl leading-none text-text">92%</p>
          ...
          <p className="mt-6 rounded-lg bg-accent p-4 text-sm leading-6 text-text">Foundation đã sẵn sàng. Dữ liệu và hành động thật được nối ở các stage tiếp theo.</p>
        </aside>
      </div>
    </section>
  );
